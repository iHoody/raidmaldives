(function($) {
    ...
    const BookingModal = {
        ...
        init: function() {
            ...
            this.checkoutUrl = diveraidBookingTraining?.checkoutUrl || '/checkout/';
            this.bindEvents();
        },
        ...
        loadEventTypes: function(postId) {
            ...
            $.ajax({
                url: diveraidBookingTraining.ajaxUrl,
                type: 'POST',
                data: {
                    action: 'diveraid_get_frontend_training_course',
                    nonce: diveraidBookingTraining.nonce,
                    post_id: postId
                },
                success: function(response) {
                    self.modal.find('.booking-modal__loading').hide();

                    if (response.success && response.data.trainingCourse.length > 0) {
                        self.eventTypes = response.data.trainingCourse;
                        self.populateSelect(response.data.trainingCourse);
        ...
        populateSelect: function(eventTypes) {
            ...
            select.html('<option value="">' + diveraidBookingTraining.strings.chooseOption + '</option>');

            eventTypes.forEach(function(type) {
                const remaining = parseInt(type.capacity) - parseInt(type.booked);
                const availabilityText = remaining > 0
                    ? ' (' + remaining + ' ' + diveraidBookingTraining.strings.spotsLeft + ')'
                    : ' (' + diveraidBookingTraining.strings.soldOut + ')';
        ...
        proceedToCheckout: function() {
            ...
            if (!productId) {
                alert(diveraidBookingTraining.strings.error);
                return;
            }

            // Add to cart and redirect to checkout
            checkoutBtn.prop('disabled', true).text(diveraidBookingTraining.strings.processing);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert(response.data.message || diveraidBookingTraining.strings.error);
                        checkoutBtn.prop('disabled', false).text(diveraidBookingTraining.strings.checkout);
                    }
                },
                error: function() {
                    alert(diveraidBookingTraining.strings.error);
